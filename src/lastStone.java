...
public class lastStone {
	public static void main(String[] args) {
		 ...
		 * converts to [1] then that's the value of the last stone. Example 2:
		 * 
		 * Input: stones = [1] Output: 1
		 */

		int[] stonesArray = { 1, 2, 4, 8, 7, 1 };
		//result should be 1
		int result = lastStoneWeight(stonesArray);
	...
	public static int lastStoneWeight(int[] stones) {
		...
		//loop until theres only one stone left or none
		while (length >= 1) {
			//sorting stones in asc order.
			Arrays.sort(stones);
			if (stones.length == 1) {
				return stones[0];
			} else {
				//assigning new value to the second last stone
				//by using the last stone to subtract the second last stone.
				//decrease the length of the loop by 1
				stones[length - 1] = stones[length] - stones[length - 1];
				length--;
			}
		}
		//returning the last stone.
		return stones[0];
	}
}
